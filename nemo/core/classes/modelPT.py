    ...
    def _setup_chakra_profiling(self):
        ...
        if self.cfg.get('chakra_profile', None) is not None:
            if self.cfg.chakra_profile.get('enabled', False):
                ...
                if self.cfg.chakra_profile.get('trace_dir', None) is None:
                    raise ValueError(
                        f'chakra profile output path ({self._memory_profile_output_path}) is not set.'
                    )

                trace_dir = Path(self.cfg.chakra_profile.get('trace_dir'))
                ...
                if self.cfg.nsys_profile.get('enabled', False):
                    logging.warning(f'Profiling with Nsys enabled will result in the Kineto trace losing GPU performance data.')
